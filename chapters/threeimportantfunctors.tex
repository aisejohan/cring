...
maximal ideal by \cref{anycontainedinmaximal},  so $(x) \subset
...
be the Jacobson
radical of $R$ (cf. \cref{Jacobson}). If $\mathfrak{I} M = M$,
then $M =
0$.
\end{exercise}
...
\cref{univpropertykernel}; it states simply that a map $M \to N$ has
...
\end{corollary}
\begin{proof}
Indeed, we know (\cref{}) that we can always get a surjection from a free
module. Since free modules are projective by \cref{projmod}, we are
done.
\end{proof}
...
\begin{proof}[Proof of \cref{tensorexists}] We will simply give
