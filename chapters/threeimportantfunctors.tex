...
Now it follows that $\hom(A, \varinjlim B_n)$ is a one-element set.
...
An object $A \in \mathcal{C}$ is said to be $\omega$-\textbf{small} if
...
\begin{definition} 
Let $\omega$ be an ordinal, and $\kappa$ a cardinal. Then $\omega$ is
...
$\kappa$, and provides an upper bound as in the definition.
\end{proof} 
...
$B_\alpha$, so does the whole thing.
\begin{proof} 
...
\[ \bigcup f^{-1}(B_\alpha) = f^{-1}(\bigcup B_\alpha) = A.  \]
...
However, $A$ has an upper bound $\widetilde{\omega} < \omega$. In particular,
...
The tensor product's key property is that it allows one to ``linearie''
