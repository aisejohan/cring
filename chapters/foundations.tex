...
underlying set is corepresentable (cf. \cref{corepresentable}). What
...
are UFDs. In particular, in  \cref{gaussianintegersareprincipal} and
\cref{polyringisprincipal}, we saw that $\mathbb{Z}[i]$ and $F[t]$ are PIDs,
...
one-element set (\cref{moduleunderlyingsetrepresentable}).
...
\cref{noetherian}, this does not happen over \emph{noetherian} rings.
...
(\cref{}), that there is a general method of reducing questions about finitely
...
object, $F'$), we find by \cref{exact-fingen} that $\coker(f)$ is finitely generated.
\end{proof} 
...
shall see in \cref{noetherian} that this does not happen over a
...
However, now, using general facts about projective modules (\cref{}), we can
...
\end{corollary} 
\begin{proof} 
Immediate from \cref{exact-finpres}
\end{proof} 
