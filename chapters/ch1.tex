...
modules, etc.) that one needs to assimilate, and the exposition of that is the
purpose of the present chapter. It is assumed that the reader has at least
some prior acquaintance with algebra, so the exposition will be occasionally
quick.
...
prove this by factoring the expression the left hand side. We can write
...
$\mathbb{Z}[\sqrt{-5}]$ of numbers of the form $a + b\sqrt{-5}$, where
...
are fundamentally different factorizations.
...
ab$, then that prime factor must divide $a$ or $b$.  This is not true,
however. There is a new idea that one must introduce.
