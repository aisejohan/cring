...
\cref{lemma-differentials-diagonal}. Moreover, the two $R$-algebra maps
...
be a solid diagram as in \cref{definition-formally-unramified}.
...
\begin{proof}
Follows from
\cref{lemma-formally-unramified-local}.
...
\noindent
In the situation of
\cref{lemma-universal-thickening}
...
\cref{lemma-formally-unramified-localize}.
\end{lemma}
...
Nakayama's \cref{lemma-NAK}). This proves that $\Omega_{B'/A} = 0$
...
Then $R \to S$ is smooth by \cref{lemma-formally-smooth-smooth}.
By \cref{lemma-characterize-formally-unramified}
...
\cref{lemma-smooth-formally-smooth}.
By \cref{lemma-characterize-formally-unramified}
...
\begin{proof}
Consider a diagram as in \cref{definition-formally-etale}.
...
\begin{proof}
Follows from \cref{lemma-characterize-formally-unramified}
and the definitions.
\end{proof}
...
\cref{lemma-trivial-differential-surjective},
and the ring map $R \to R/I$
is of finite type. If $I$ is a finitely generated ideal then $R \to R/I$
is of finite presentation.

\medskip\noindent
Ad (5). See discussion following \cref{definition-etale}.

\medskip\noindent
Ad (6). In this case $\Omega_{S/R}$ is a finite $S$-module (see
\cref{lemma-differentials-finitely-generated}) and hence there
exists a $g \in S$, $g \not \in \mathfrak q$ such that
$(\Omega_{S/R})_g = 0$. By \cref{lemma-differentials-localize}
this means that $\Omega_{S_g/R} = 0$ and hence $R \to S_g$ is
unramified as desired.

\medskip\noindent
Ad (7). Use Nakayama's lemma (\cref{lemma-NAK}) to see that
...
\cref{lemma-differentials-base-change}.
...
$J/J^2 = 0$, see
\cref{lemma-differentials-diagonal}.
...
We win by \cref{lemma-ideal-is-squared-union-connected}.
\end{proof}
...
\cref{lemma-unramified}.
By
\cref{lemma-characterize-smooth-over-field}
...
$\kappa(\mathfrak p)$ by \cref{lemma-etale-over-field}.
...
\cref{lemma-isolated-point-fibre} part (2) using
\cref{lemma-unramified-at-prime}.
\end{proof}
...
\begin{proof}
By \cref{lemma-unramified} (8) it suffices to show that
$\Omega_{S \otimes_R \kappa(\mathfrak p) / \kappa(\mathfrak p)}$
...
\cref{lemma-unramified-quasi-finite}.
By \cref{lemma-quasi-finite-open-integral-closure}
...
(see \cref{lemma-finite-dimensional-algebra}) and
so a finite product of local rings
$$
S \otimes_R \kappa(\mathfrak p) = \prod\nolimits_{i = 1}^n A_i
$$
see \cref{proposition-dimension-zero-ring}. One of the factors,
say $A_1$, is the local ring $S_{\mathfrak q}/\mathfrak pS_{\mathfrak q}$
which is isomorphic to $\kappa(\mathfrak q)$,
see \cref{lemma-unramified-at-prime}. The other factors correspond to
...
\cref{lemma-unique-prime-over-localize-below} (we have
going up for $S' \to S$ by \cref{lemma-integral-going-up}
...
see \cref{lemma-unramified-at-prime}.
Hence a fortiori
...
Hence by Nakayama's \cref{lemma-NAK} applied to the
...
\medskip\noindent
Step 7. By \cref{lemma-local-isomorphism} there exists a $g' \in S'$,
...
see \cref{lemma-standard-etale}) and $\varphi(g) \not \in \mathfrak q$.
...
\cref{lemma-etale-makes-quasi-finite-finite-variant}. Thus we may
...
(see \cref{lemma-unramified-quasi-finite}).
Say $\mathfrak q = \mathfrak r_1$.
By \cref{lemma-unramified-at-prime} we see that
...
Also, by \cref{lemma-unique-prime-over-localize-below}
(which applies to $R \to A_1$ because a finite ring map satisfies going up by
\cref{lemma-integral-going-up})
we have $(A_1)_{\mathfrak r_1} = (A_1)_{\mathfrak p}$.
It follows from Nakayama's \cref{lemma-NAK}
