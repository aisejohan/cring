...
A filtration on a ring $R$ is a series of ideals $F:\cdots I_n\subseteq
I_{n-1}\subseteq\cdots I_1\subseteq I_0=R$. For example, given an ideal $I$, we
can take powers of $I$ to generate a filtration.
\begin{definition} Given a filtration $F$ on a ring $R$ and a module $M$, the
graded associated module $gr_F(M)$ is $\bigoplus_{n=0}^\infty I_n M/I{n+1}M$
\end{definition}
