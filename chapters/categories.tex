...
For instance, what matters here is not the theorem, but the \emph{definition of an
...
Start with a small category $I$: recall that this means that the objects of $I$ form a set. One is supposed to picture
...
\in \mathcal{C}$ equipped with maps $F_i \to X, \forall i \in I$ such that for all maps $i \to
...
Let us consider the coproduct $\sqcup_{i \in I, f} Fi$, where $f$ ranges over all
...
with a morphism $B \to A$, then $B$ is weakly initial too. So we are going to take
...
\end{proposition} 
\begin{proof} 
Let $A'$ be this equalizer; it is endowed with a morphism $A'\to A$. Then let us recall what this means. For any two
...
homotopy), any contravariant functor that satisfies two relatively mild conditions (a
