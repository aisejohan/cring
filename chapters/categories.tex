...
\mathcal{C}$, then $F(1_X)$ is the identity morphism $1_{F(X)}: F(X) \to F(X)$.The second is that if $A \stackrel{f}{\to} B \stackrel{g}{\to} C$ are
...
\stackrel{F}{\to}\mathcal{D}$ (between categories $\mathcal{C}, \mathcal{D}$) is
similar
...
we have two maps of rings $s:R\rightarrow S$ and $q:R\rightarrow Q$. Then we can
fit this information together
...
\end{proposition} 
\begin{proof} 
Let $A'$ be this equalizer; it is endowed with a morphism $A'\to A$. Then let us
recall what this means. For any two
