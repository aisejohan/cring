...
flatness, $\im(f) = 0$ by \cref{easyffcriterion}.
...
\cref{easyffcriterion}, we are done again.
\end{proof} 
...
\cref{easyffcriterion}. So we get one direction.
...
shall use this and \cref{easyffcriterion} to prove that $M$ is f.f.
...
\cref{easyffcriterion}, it follows that $M$ is f.f.
\end{proof} 
...
\add{this section needs to be considerably expanded: define Tor, prove basic
properties, etc.?}
...
\]
We apply $A \otimes_{\mathbb{Z}/m\mathbb{Z}} \cdot$ and truncate. Since 
$S \otimes_R R \cong S$, we see that $A \otimes_{\mathbb{Z}/m\mathbb{Z}} \mathbb{Z} /m\mathbb{Z} \cong A$ and
